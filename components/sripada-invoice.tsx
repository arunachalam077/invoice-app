...
export default function SripadaInvoice({ invoice, onBack, onSendEmail, onEdit }: SripadaInvoiceProps) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      await inlineImages(element)
      // Generate PDF and compress if needed to avoid large request body errors
      const generatePdfBase64 = async (rootEl: HTMLElement) => {
        const html2pdf = (await import("html2pdf.js")).default
        const MAX_BASE64 = 6000000 // ~4.5MB binary (base64 ~6M chars)

        const qualityOptions = [0.98, 0.9, 0.8, 0.7, 0.6]
        const scaleOptions = [2, 1.8, 1.6, 1.4, 1.2]
        ...
        for (let q of qualityOptions) {
          for (let s of scaleOptions) {
            try {
              ...
              console.log(`[v0] Tried q=${q}, s=${s} → base64 len=${base64.length}`)
              if (base64.length <= MAX_BASE64) return base64
            } catch (err) {
              console.warn("[v0] PDF try failed for q/s", q, s, err)
              continue
            }
          }
        }
        ...
        const finalWorker = (await import("html2pdf.js")).default()
          .set({
            ...
            html2canvas: { scale: 1.2, useCORS: true, allowTaint: true, backgroundColor: "#ffffff" },
            jsPDF: { orientation: "portrait", unit: "mm", format: "a4", compress: false },
          })
          .from(rootEl)
